...
def cos_sim_acc(training_data, input_data, num_recommendation, training_set = 0):
    ...
    # input_data = input_data.drop(['channel_title'], axis = 1)
    # input_decision = input_data['decision']
    # input_data = input_data.drop(['decision'], axis = 1)
...
test_data = test_data.head(2000).reset_index(drop=True)
training_data = training_data.head(10000).reset_index(drop=True)
